...
| 2  | 一元一次方程  | af(x) + b = c            | abc是整数         |
| 3  | 函数相加    | f(x) + g(x) = c          | c 是常数, 解 x     | 
| 4  | 函数相除    | f(x) / g(x) = c          | c 是常数, 解 x     |
| 5  | 函数相乘    | f(x)g(x) = c             | c是常数, 解 x      |
| 6  | 二元一次方程  | f(x,y)=a, g(x,y)=b       | a,b是常数, 解 x    |
| 7  | 二元一次方程  | f(x,y) / g(x,y) = a      | a 是常数, 解 y     |
| 8  | 二元一次方程  | f(x,y) = af(x) + bf(y)   | ab是常数, 解 y     |
| 9  | 复合函数    | f(f(x)) = a              | a是常数, 解 x      |
| 10 | 复合函数    | f(f(x)) = f(x)           | 解 x            |
| 11 | 复合函数    | f(f(x), y) = 0           | 解 x            |
| 12 | 反函数     | f^{-1}(x) =              | 求表达式           |
| 13 | 反函数     | f^{-1}f(x) + f(x) = 0    | 解 x            |
| 14 | 反函数     | f^{-1}(x)f(x) =          | 求表达式           |
| 15 | 反函数复合函数 | f(f^{-1}(x)) =           | 求表达式           |
| 16 | 反函数复合函数 | f^{-1}(f(x)) = f^{-1}(x) | 解方程            |
